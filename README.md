...
Called with `<n-copy></n-copy>` and additional attributes listed below.

| Variable | Description | Default | Accepts | Note |
| -------- | -------- | -------- | -------- | -------- |
| *position* | Sets the position of the n-copy element. | "unset" | "top right", "top left", "top", "bottom right", "bottom left", "bottom", "left", "right", "unset" | All positions outside of "unset" will be assigned absolute positioning and should be placed inside of a relative element. |
...
| *successColor* | When copied, sets the button color to the specified value for 2s | null | Any CSS-valid color string | |
| *dark* | Sets the button text to white. | false | true, false | |
| *background* | Sets the button's background. | null | Any CSS-valid background | |
